...
INSTRUCTIONS = (
    "To use certbot-dns-route53-custom, configure credentials as described at "
...
class Authenticator(common.Plugin, interfaces.Authenticator):
    ...
    def add_parser_arguments(cls, add: Callable[..., None]) -> None:
        add('zone-id',
            help=('The ID of the Hosted Zone in Route 53 that is responsible for the domain.' +
                  'This will be determined automatically if not specified.'),
            type=str,
            default=None)
    ...
    def _verify_zone_id_for_domain(self, zone_id: str, domain: str) -> None:
        try:
            response = self.r53.get_hosted_zone(Id=zone_id)
        except (self.r53.exceptions.NoSuchHostedZone, self.r53.exceptions.InvalidInput):
            raise errors.PluginError(
                "Unable to find a Route53 hosted zone with ID {0}".format(zone_id)
            )

        zone = response["HostedZone"]
        if zone["Config"]["PrivateZone"]:
            raise errors.PluginError(
                "Route53 hosted zone with ID {0} is a private zone".format(zone_id)
            )

        hosted_zone_name = zone["Name"].rstrip(".")
        candidate_labels = hosted_zone_name.split(".")
        target_labels = domain.rstrip(".").split(".")
        if candidate_labels != target_labels[-len(candidate_labels):]:
            raise errors.PluginError(
                "Route53 hosted zone {0} ({1}) is not compatible with domain {2}".format(
                    zone_id,
                    hosted_zone_name,
                    domain
                )
    ...
    def _change_txt_record(self, action: str, validation_domain_name: str, validation: str) -> str:
        zone_id = self.conf('zone_id')
        if zone_id is None:
            zone_id = self._find_zone_id_for_domain(validation_domain_name)
        else:
            self._verify_zone_id_for_domain(zone_id, validation_domain_name)
